...
The file contains module and feature branches, then one topic per case with its ID, title, description, priority, type, preconditions, steps, and expected results. When a projection carries `canonical-id`, that stable ID is shown instead of Casebook's compatibility ID. It is an editable handoff copy and never updates the YAML source.
...
For `ai-test-case-sdd` projections, the download checks the bound canonical Case Set hash and the projected case-content hash. Older projections without this binding must be regenerated before export. Review-page edits must be applied to the canonical Case Set and projected again.
